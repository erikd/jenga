...
-- This module was stolen from https://github.com/ambiata/mafia and modified
-- slightly for use in Jenga.

module Jenga.Git.Process
  ( -- * Inputs
    File
  , Directory
  , Argument
...
fromProcess' p = (cmd, args, cwd, env)
  where
    ...
    args = processArguments p
    cwd  = processDirectory p

    env  = fmap Map.toList (processEnvironment p)
